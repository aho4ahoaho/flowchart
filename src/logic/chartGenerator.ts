...
export const chartGenerator = (
	...
	let name = func.name == "main" ? "START" : func.name;
	let rname = func.name == "main" ? "END" : "RTS";
	// サイズを定義する
	let size = {
	...
	// 描画するクラスをインスタンス化する
	let chart = new ChartShape(ctx, style ?? {});

	// 開始記号を描画する
	chart.terminator(size.offset.x, size.offset.y, size.width, size.height, name);
	chart.arrow(
	...
	const nodeProcesser = (nodes: Item[]) => {
		nodes.forEach((node, index) => {
			...
			if (typeof node == "string") {
				...
				// 条件の形を描画する
				let x = nest * (size.width + size.margin) + size.offset.x;
				let y = step * (size.height + size.margin) + size.offset.y;
				...
				// True側からの戻りの矢印を描画する
				(() => {
					let x = (nest + 1) * (size.width + size.margin) + size.offset.x;
					let y = (s1 - 1) * (size.height + size.margin) + size.offset.y;
					...
					const y1 =
						...
						(size.margin / 3) * 2;
					const x2 =
						nest * (size.width + size.margin) + size.offset.x + size.width / 2;
					const y2 = y1;
					chart.preProcess();
					ctx.moveTo(x0, y0);
					ctx.lineTo(x1, y1);
				...
				(() => {
					let x = nest * (size.width + size.margin) + size.offset.x;
					let y = (s2 - 1) * (size.height + size.margin) + size.offset.y;
					const x0 = x + size.width / 2;
					const y0 = y + size.height;
					const x1 = x0;
					const y1 = step * (size.height + size.margin) + size.offset.y;
					if (index != nodes.length - 1) {
				...
				(() => {
					...
					const x2 = x1;
					const y2 =
						s0 * (size.height + size.margin) + size.offset.y - size.margin / 3;
...
class ChartShape {
	...
	constructor(ctx: CanvasRenderingContext2D, style: Partial<ChartStyle> = {}) {
		this.ctx = ctx;
		this.style = {
	...
	// 処理の形を描画する
	process(x: number, y: number, width: number, height: number, text?: string) {
		this.preProcess();
	...
	// 条件の形を描画する
	desicion(x: number, y: number, width: number, height: number, text?: string) {
		const point = [
			{ x: x + width / 2, y: y },
