...
export const searchSentence = (context: string): Item[] => {
	...
	while (i < context.length) {
		...
		if (context.substring(i, i + 2) == "if") {
			// if文の範囲を見つける
			let startIndex = context.indexOf("{", i + 2);
			let endIndex = searchScopeEnd(context, startIndex);
			if (startIndex !== -1 && endIndex !== -1) {
				// if文の情報をnodesに追加
				let node: Scope = {
					type: "if",
					condition: context.substring(i + 3, context.indexOf("{", i + 2) - 1),
					node: searchSentence(context.substring(startIndex + 1, endIndex + 1)),
				};
				...
				if (context.substring(0, 4) == "else") {
					// else文の範囲を見つける
					let startIndex = context.indexOf("{", 4);
					let endIndex = searchScopeEnd(context, startIndex);
		...
		if (context.substring(i, i + 4) == "loop") {
			// loop文の範囲を見つける
			let startIndex = context.indexOf("{", i + 4);
			let endIndex = searchScopeEnd(context, startIndex);
			if (startIndex !== -1 && endIndex !== -1) {
				// loop文の情報をnodesに追加
				nodes.push({
					type: "loop",
					condition: context.substring(i + 5, context.indexOf("{", i + 4) - 1),
					node: searchSentence(context.substring(startIndex + 1, endIndex + 1)),
				} as Scope);
		...
		// 次の";"までを一つのノードとする
		let endIndex = context.indexOf(";", i);
		let node = context.substring(i, endIndex);
		nodes.push(node);
